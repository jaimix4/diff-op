...
# Store the raw data so we don't permanently overwrite it when smoothing
dn_dR_raw = data[:, 4]
dTe_dR_raw = data[:, 5]
dTi_dR_raw = data[:, 6]
Gamma_raw = data[:, 10]
...
plt.subplots_adjust(bottom=0.28, right=0.85) # Increased bottom margin for extra slider

init_N = 5
init_w = [1.0, 1.0, 1.0, 1.0]

# Calculate initial unfiltered qe and qi for setup
...
# ==========================================
# 5. UI SLIDERS (Smoothing, Regions & Weights)
# ==========================================
...
def update(val):
    ...
    # 1. Apply Gaussian Filter if sigma > 0
    if sigma > 0.1:
