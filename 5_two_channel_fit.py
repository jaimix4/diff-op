...
# ==========================================
def optimize_dynamic(N_regions, w1, w2, w3, w4, dn_curr, dTe_curr, dTi_curr, G_curr, qe_curr, qi_curr, rho_c_min, rho_c_max, use_convection):
    ...
    def get_bounds(r_check):
        """Returns bounds based on whether the current location allows convection."""
        if not use_convection:
            c_bnd = (0.0, 0.0)
        else:
            # If the check location is within the allowed convection range, bounds are open (None, None).
            # Otherwise, rigidly lock C to 0 by bounding it at (0.0, 0.0).
            c_bnd = (None, None) if (rho_c_min <= r_check <= rho_c_max) else (0.0, 0.0)
        return ((None, None), (0.0, 20.0), c_bnd, 
                (None, None), (0.0, 20.0), c_bnd)
    ...
    if N_regions >= NUM_POINTS:
        ...
        for i in range(len(edges) - 1):
            ...
            # Use the middle of the region to check if convection is allowed
            r_mid = (r_min + edges[i+1]) / 2.0
            bnds = get_bounds(r_mid)
...
# Increased bottom margin to fit the 7th slider cleanly
plt.subplots_adjust(bottom=0.32, right=0.85)

init_N = 4
init_w = [1.0, 1.0, 1.0, 1.0]
init_rho_c = (rho.min(), rho.max()) # Start with convection allowed everywhere
init_use_conv = True # Convection toggle initial state

init_qe_data = Qe_total_raw - (1.5 * Gamma_raw * Te * e_charge)
init_qi_data = Qi_total_raw - (1.5 * Gamma_raw * Ti * e_charge)

res = optimize_dynamic(init_N, *init_w, dn_dR_raw, dTe_dR_raw, dTi_dR_raw, Gamma_raw, init_qe_data, init_qi_data, init_rho_c[0], init_rho_c[1], init_use_conv)
...
ax_conv_t = plt.axes([0.68, 0.175, 0.12, 0.03], frameon=False) # Toggle Button
...
# RangeSlider takes two values to define the active window
...
def update(val):
    ...
    r = optimize_dynamic(N_reg, *w, dn_curr, dTe_curr, dTi_curr, Gamma_curr, qe_curr, qi_curr, rho_c_min, rho_c_max, use_conv)
