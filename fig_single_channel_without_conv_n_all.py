...
for i in range(NUM_POINTS):
    ...
    comps = compute_model(res.x[0], res.x[1], n[i], Te[i], dn_dR[i], dTe_dR[i])
    out['G_tot'][i], out['q_tot'][i] = comps[0], comps[1]
    out['G_n'][i], out['G_T'][i] = comps[2], comps[3]
    out['q_n'][i], out['q_T'][i] = comps[4], comps[5]
...
# --- PANEL 1: Parameters ---
# Plotted together on a single unified y-axis. Changed from step() to plot() for point-by-point.
...
axs[0].set_title(r"Sinc Operator Electron Channel without Convection, Point-by-Point")
...
plt.savefig("single_channel_without_conv_point_by_point.png", dpi=300, bbox_inches='tight')
plt.show()